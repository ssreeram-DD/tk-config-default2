...
class MayaPublishFilesCustomPlugin(HookBaseClass):
    """
    ...
    def _camera_naming(self, group_nodes):
        ...
        if main_cam:
            if len(main_cam) > 1:
                ...
                # cmds.confirmDialog(title="ERROR!", message="Incorrect camera name! Should be CAM.", button=['Ok'])
                return False
        else:
    ...
    def validate(self, task_settings, item):
        ...
        status = True
        status = self._node_naming(group_nodes, groups) and status
        status = self._camera_naming(group_nodes) and status

        if not status:
            return status
        # node_names = self._node_naming(group_nodes, groups)
        # cam_name = self._camera_naming(group_nodes)
        # hierarchy = self._check_hierarchy(group_nodes)
        # name_prefix, children = self._track_geo_child_naming(group_nodes[-1])
        # locks = self._locked_channels(children)
        #
        # if not(node_names and cam_name and hierarchy and name_prefix and locks):
        #     return False

        return super(MayaPublishFilesCustomPlugin, self).validate(task_settings, item)
